...
LevelSelect = (function() {
  ...
  LEVEL_TEMPLATE = "    <div class='level' data-level='<%= index %>'>      <h2><%= config.name %></h2>      <span class='score'><%= highscore.score %></span>      <span class='distance'><%= highscore.distance %></span>      <div class='stars'>        <div class='star' id='star0'></div>        <div class='star' id='star1'></div>        <div class='star' id='star2'></div>      </div>    </div>  ";
  ...
  LevelSelect.prototype.start = function() {
    ...
    $(this.domElement).on('click', '[data-level]', function(e) {
      ...
      if (_this.levelUnlocked(levelIndex)) {
        return _this.playLevel(levelIndex);
      } else {
        return alert('Locked!');
      }
    });
  ...
  LevelSelect.prototype.buildLevels = function() {
    var config, distance, highscore, index, level, levelIndex, _i, _j, _len, _len1, _ref, _results;
    $(this.domElement).find('[data-level]').remove();
    _results = [];
    for (levelIndex = _i = 0, _len = LEVELS.length; _i < _len; levelIndex = ++_i) {
      config = LEVELS[levelIndex];
      highscore = this.highscores.get(levelIndex);
      ...
      _results.push($(this.domElement).find('.levels').append(level));
    }
    return _results;
  };
  ...
  LevelSelect.prototype.playLevel = function(levelIndex) {
    var levelConfig;
  ...
  LevelSelect.prototype.gameEnded = function(levelIndex, score, distance) {
    this.highscores.set(levelIndex, score, distance);
    this.game.destroy();
    this.game = void 0;
    return this.show();
  };
...
NullRenderComponent = (function() {
  ...
  NullRenderComponent.prototype.updateTimer = function(remainingTime, totalTime) {
    return true;
  };

  NullRenderComponent.prototype.buildGameBoard = function(length, markers) {
    return true;
  };
  ...
  NullRenderComponent.prototype.buildCharacterOnColumn = function(columnIndex, oldColumnIndex) {
    return true;
  };
...
DomRenderComponent = (function(_super) {
  var SPRITES, createElementForBall, formatDistance, formatScore, formatTime, getTimestamp, lengthToPx;

  __extends(DomRenderComponent, _super);

  SPRITES = ['sheep1', 'sheep2', 'flower', 'flower', 'flower', 'flower', 'flower', 'tree', 'tree'];
  ...
  DomRenderComponent.prototype.updateTimer = function(remainingTime, totalTime) {
  ...
  DomRenderComponent.prototype.buildGameBoard = function(length, markers) {
    var currentDistance, distance, frame, html, marker, markerOffset, progress, score, sprite, spriteIndex, spriteOffsetLeft, spriteOffsetTop, time, timer, _i, _len;
    this.length = length;
    this.parent.find('.game-over').hide();
    this.parent.find('.board').remove();
    this.board = $('<div/>');
    this.board.addClass('board');
    this.height = lengthToPx(this.length) + (window.innerHeight * 2);
    this.expectedOffset = lengthToPx(this.length) + 200 + window.innerHeight;
    this.board.css('height', this.height);
    this.offset = -lengthToPx(this.length) - window.innerHeight;
    this.board.css('top', this.offset);
    currentDistance = 0;
    while (lengthToPx(currentDistance) <= this.height) {
    ...
    for (_i = 0, _len = markers.length; _i < _len; _i++) {
      ...
      markerOffset = lengthToPx(this.length - distance) + 170 + window.innerHeight;
      marker.css('top', markerOffset);
      this.board.append(marker);
    }
    this.columns = $('<div/>');
    this.columns.addClass('columns');
    this.columnsOffset = lengthToPx(this.length) + 270 + window.innerHeight;
  ...
  DomRenderComponent.prototype.popBallFromColumn = function(ballObject, columnIndex) {
    ...
    ball.addClass('pop');
    return this.removeBallInMs(ballObject.id, 150);
  };
  ...
  DomRenderComponent.prototype.destroyBallFromColumn = function(ballObject, columnIndex) {
    ...
    return this.removeBallInMs(ballObject.id, 450);
  };

  DomRenderComponent.prototype.buildCharacterOnColumn = function(columnIndex, oldColumnIndex) {
    var character;
  ...
  DomRenderComponent.prototype.showGameOverScreen = function(finalScore, finalDistance, stars, returnToLevelSelectCallback) {
    var distance, gameover, index, _i, _len,
      _this = this;
    ...
    for (index = _i = 0, _len = stars.length; _i < _len; index = ++_i) {
      distance = stars[index];
...
Character = (function() {
  ...
  Character.prototype.moveToColumn = function(column) {
    this.options.renderComponent.buildCharacterOnColumn(column, this.column);
...
Game = (function() {
  ...
  Game.prototype.start = function() {
    this.newRowIntervalMs = (1 / this.options.config.speed) * 10000;
  ...
  Game.prototype.gameLoop = function() {
    var deltaLength, deltaMs, markerDistance, remainingTime, timeElapsed;
    this.distance = this.rowsGenerated - this.countRows();
    if (this.distance < 0) {
      this.distance = 0;
    }
    timeElapsed = (getTimestamp() - this.startTime) / 1000;
    if ((timeElapsed - this.addedTime) > this.options.totalTime) {
      timeElapsed = this.options.totalTime;
      this.triggerGameOver();
    }
    remainingTime = (this.options.totalTime + this.addedTime) - timeElapsed;
    this.options.renderComponent.updateTimer(remainingTime, this.options.totalTime);
    markerDistance = this.options.config.stars[this.nextMarkerIndex];
    if (this.distance > markerDistance) {
      this.nextMarkerIndex++;
      if (this.nextMarkerIndex === this.options.config.stars.length) {
        this.distance = this.options.config.length;
        return this.triggerGameOver();
      } else {
        this.addedTime += this.options.totalTime - remainingTime;
      }
    }
    ...
    deltaLength = deltaMs * (1 / this.newRowIntervalMs);
  ...
  Game.prototype.buildGameBoard = function() {
    var x, _i, _ref;
    this.options.renderComponent.buildGameBoard(this.options.config.length, this.options.config.stars);
    this.balls = [];
    this.characterBalls = [];
  ...
  Game.prototype.buildRow = function() {
    ...
    this.rowsGenerated++;
    return this.buildNextRowAt = getTimestamp() + this.newRowIntervalMs;
  };
  ...
  Game.prototype.triggerGameOver = function() {
    ...
    return this.options.renderComponent.showGameOverScreen(this.score, this.distance, this.options.config.stars, this.returnToLevelSelect);
  };
